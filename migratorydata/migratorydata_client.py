...
class MigratoryDataMessage:
	def __init__(A,_subject,_content,closure=_A,qos=QoS.GUARANTEED,retained=True,reply_subject=_A):A._subject=_subject;A._content=_content;A._closure=closure;A._reply_to_subject=reply_subject;A.aGP=qos;A._retained=retained;A._message_type=MessageType.UPDATE;A._seq=_A;A._epoch=_A;A._compression=_A
...
class MigratoryDataClient:
	NOTIFY_SERVER_UP='NOTIFY_SERVER_UP';NOTIFY_SERVER_DOWN='NOTIFY_SERVER_DOWN';NOTIFY_DATA_SYNC='NOTIFY_DATA_SYNC';NOTIFY_DATA_RESYNC='NOTIFY_DATA_RESYNC';NOTIFY_SUBSCRIBE_ALLOW='NOTIFY_SUBSCRIBE_ALLOW';NOTIFY_SUBSCRIBE_DENY='NOTIFY_SUBSCRIBE_DENY';NOTIFY_PUBLISH_OK='NOTIFY_PUBLISH_OK';NOTIFY_PUBLISH_FAILED='NOTIFY_PUBLISH_FAILED';NOTIFY_MESSAGE_SIZE_LIMIT_EXCEEDED='NOTIFY_MESSAGE_SIZE_LIMIT_EXCEEDED';NOTIFY_PUBLISH_DENIED='NOTIFY_PUBLISH_DENIED';wJC='NOTIFY_CONNECT_OK';CIZ='NOTIFY_CONNECT_DENY';CONSTANT_WINDOW_BACKOFF='CONSTANT_WINDOW_BACKOFF';TRUNCATED_EXPONENTIAL_BACKOFF='TRUNCATED_EXPONENTIAL_BACKOFF'
	def __init__(A):A._client_impl=eod()
	def connect(A):A._client_impl.Qgg()
	def disconnect(A):A._client_impl.UZw()
	def set_listener(B,listener):
		...
		B._client_impl.bYY(A)
	def get_listener(A):return A._client_impl.SJR()
	...
	def set_entitlement_token(B,entitlement_token):
		...
		B._client_impl.uAz(A)
	def set_servers(A,servers):B=servers;A._check_if_is_string_list(B,'set_servers');A._client_impl.zzG(B)
	...
	def subscribe(A,subject_list):B=subject_list;A._check_if_is_string_list(B,'subscribe');A._client_impl.Poh(B,0)
	def subscribe_with_history(A,subject_list,number_of_historical_messages):
		...
		A._client_impl.Poh(B,C)
	def unsubscribe(A,subject_list):B=subject_list;A._check_if_is_string_list(B,'unsubscribe');A._client_impl.uJm(B)
	def publish(B,message):
		...
		B._client_impl.JWd(A)
	def set_encryption(B,encryption_bool):
		A=encryption_bool
		if type(A)is not bool:raise TypeError('Argument for set_encryption must be of type bool')
		B._client_impl.gPn(A)
	def get_subjects(A):return A._client_impl.yzw()
	def check_for_messages(A):return A._client_impl.check_for_messages()
from .core.client_impl import eod